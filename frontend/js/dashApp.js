...
// =========================
// Main Script: Dashboard & Batch Management
// =========================
document.addEventListener('DOMContentLoaded', () => {
  // =========================
  // Konstanta API
  // =========================
  const API_BASE_URL = 'http://localhost:3001/api/transactions'; // Endpoint transaksi
  const AUTH_API_URL = 'http://localhost:3001/auth'; // Endpoint auth
  ...
  // 3. Handler klik tombol di tabel batch (analisis, lihat hasil, hapus)
  ...
  async function fetchUserData() {
    try {
      ...
      if (res.ok) {
        ...
        if (userNameElement) {
          userNameElement.textContent = data.name || 'Pengguna';
        }
      }
    } catch (error) {
